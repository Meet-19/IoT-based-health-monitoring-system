...
#define SC_ADDRESS 0x3C
#define SC_HEIGHT 64 
#define SC_WIDTH 128
#define OLED_RESET -1

#define DS18B20 5
#define ms_reporting_period 10000

Adafruit_SSD1306 display(SC_WIDTH, SC_HEIGHT, &Wire, OLED_RESET);
WebServer server(80); 
float BodyTemperature,BPM,SpO2;

const char* ssid="your network name"; //ssid username
...
void setup() { 
  ...
  if(!display.begin(SSD1306_SWITCHCAPVCC, SC_ADDRESS)) {
  ...
  if (!POX.begin()) {
    ...
    POX.setOnBeatDetectedCallback(Input_Detected);
  }
...
void loop() { 
  ...
  if ( millis() - msLastReport > ms_reporting_period ){
